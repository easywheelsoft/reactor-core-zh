...
获得 `Flux` 或 `Mono` 并不一定意味着它要在特定的线程运行。相反，大多数操作符继续在上一个操作符执行的 `线程` 中工作。除非指定，否则最上面的操作符（源）本身运行在调用了 `subscribe()` 的 `线程` 上。下面的示例在一个新的线程上运行 `Mono`：
...
在Reactor中，执行模型和执行的位置由使用的 `Scheduler` 决定。https://projectreactor.io/docs/core/release/api/reactor/core/scheduler/Scheduler.html[`Scheduler`] 具有类似于 `ExecutorService` 的调度职责，但有一个专门的抽象使其可以做更多的事情，尤其是作为一个时钟和更广的范围实现（虚拟时间测试，波动或立即调度等）。
...
Reactor提供了两种方式来切换响应式链中的执行上下文（或 `Scheduler` ）: `publishOn` 和 `subscribeOn`。两者都取一个 `Scheduler`，并让你将执行上下文切换到该调度器。但是 `publishOn` 在链中的位置是很重要的，然而 `subscribeOn` 的位置却是无关紧要的。要理解这个区别，你首先要记住<<reactive.subscribe,nothing happens until you
subscribe>>。
...
TIP: 快速查看关于错误处理的操作符，参看<<which.errors,the relevant operator decision tree>>。
...
最后一个与命令式编程对应的是清理，通过使用 “`使用 `finally` 块或Java 7以上支持的 “`try-with-resource`” 语法清理资源`”，如下所示：
...
伴随的 `Flux` 是一个 `Flux<Throwable>` ，它被传递给一个 `Function`，且作为 `retryWhen` 的唯一的参数。作为用户，你定义该函数并使其返回新的 `Publisher<?>`。重试周期如下：

. 每次发生错误时（提供重试的可能性），错误都会被发送到伴随的 `Flux` 中，而这个 `Flux` 已经被你的函数装饰过了。这里的 `Flux` 可以看到目前为止所有的尝试。
...
<3> 这里，我们认为前三个错误是可以重试的 (`take(3)`)，然后放弃。
====
...
TIP: 你可以通过使用<<hooks-internal,hook>>来调整在传递给 `onError` 之前的 `Exception`。
...
. 捕获异常，将其封装成一个_不检查_的异常，然后将其抛出（中断序列）。`Exceptions` 工具类可以帮助你解决这个问题（接下来我们会讲到这个问题）。
